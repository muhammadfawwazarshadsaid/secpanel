...
class DatabaseHelper {
  ...
  Future<String> generateCustomExportJson({
    ...
    final endpoint =
        '/export/custom?panels=$includePanelData&users=$includeUserData&role=${currentUser.role.name}&company_id=${currentUser.id}';
  ...
  Future<String> generateFilteredDatabaseJson(
    ...
    final tables = tablesToInclude.entries
        ...
        .join(',');
    final endpoint =
        '/export/database?tables=$tables&role=${currentUser.role.name}&company_id=${currentUser.id}';
    final data = await _apiRequest('GET', endpoint);
    return JsonEncoder.withIndent('  ').convert(data);
  }
